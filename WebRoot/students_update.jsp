		...
		function setStudentInfo(stdnum){
			var params = {
				...
				$.ajax({
					...
					success : function(json){
						var obj = $.parseJSON(json);
						if(obj.result["stdnum"] == null){
							alert("学号错误，无法更改信息");
						}
						else{
							document.getElementById("cardID").setAttribute("value", obj.result["cardID"]);
							document.getElementById("name").setAttribute("value", obj.result["name"]);
							document.getElementById("email").setAttribute("value", obj.result["email"]);
							document.getElementById("qq").setAttribute("value", obj.result["QQ"]);
							document.getElementById("password").setAttribute("value", obj.result["password"]);
							document.getElementById("phone").setAttribute("value", obj.result["phone"]);
							selectSOP("major", parseInt(obj.result["major_id"]));
							selectSOP("grade", parseInt(obj.result["grade_id"]));
						}
					},
					error : function(json){
						alert("error " + json);
		                    	...
		                    	<form class="form-horizontal" role="form" >
		                    		<fieldset disabled>
			                    		<div class="form-group" id="inputStudentNumber">
											...
											<div class="col-sm-3">
										    	<input type="text" class="form-control" id="studentNumber" type="text" value = "${stdnum}">
										    	</input>
										    </div>
										</div>
									</fieldset>
									...
									<div class="form-group" id="inputPassword">
										...
										<div class="col-sm-3">
									    	<input type="text" class="form-control" id="password" type="text" 
									...
									<div class="form-group" id="output">
			                            ...
			                            <div class="col-sm-3">
			                                <input type="button" class="btn btn-success" onclick="checkForm()" value="更改用户信息">
